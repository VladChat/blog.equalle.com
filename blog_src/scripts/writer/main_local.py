# ============================================================
# File: blog_src/scripts/writer/main_local.py
# Full path: C:\Users\vladi\Documents\blog.equalle.com\blog_src\scripts\writer\main_local.py
# ============================================================
...
# === legacy helpers kept as in old working main.py ===
...
# === NEW sources (вместо RSS): категории + CSE + YouTube ===
from .topics_pairs import get_next_pair, record_used_pair     # берём core→longtail из НОВОГО categories.json
...
def _load_config(project_root: Path) -> dict:
    print("[eQualle CONFIG][LOAD] 🔍 Searching for writer_config.json...")
    candidates = [
        project_root / "blog_src" / "scripts" / "writer" / "writer_config.json",
        project_root / "blog_src" / "config" / "writer_config.json",
        project_root / "writer_config.json",
    ]
    for c in candidates:
        if c.exists():
            print(f"[eQualle CONFIG][FOUND] ✅ {c}")
            with c.open("r", encoding="utf-8") as f:
                cfg = json.load(f)
            print(f"[eQualle CONFIG][INFO] content_dir={cfg.get('content_dir')}, data_dir={cfg.get('data_dir')}, category_dir={cfg.get('category_dir')}")
            return cfg
    print("[eQualle CONFIG][WARN] ⚠️ Config not found, using defaults.")
    return {
        "content_dir": "blog_src/content/posts",
        "category_dir": "blog_src/content/categories",
        "data_dir": "blog_src/data",
        "min_words": 1200,
        "google_cse": {"results": 6, "gl": "us", "lr": "lang_en"},
    }
...
def main() -> None:
    print("────────────────────────────────────────────")
    print("[eQualle Writer][INIT] 🚀 Starting in CSE seed→longtail mode")
    ...
    # === Пара core→longtail из НОВОГО categories.json ===
    ...
    # Категория-директория (_index.md)
    ...
    if find_video_for_article:
        ...
        try:
            ...
            if v and isinstance(v, dict) and v.get("id"):
                # Заголовок секции, если пустой — сгенерить по старой схеме
    ...
    # === Prompt (как было): видео передаём ТОЛЬКО если найдено ===
