...
try:
    from .rss_video_fetch import find_video_for_article
except Exception:
...
def _strip_any_llm_video_sections(md: str) -> str:
    ...
    if new_md != md:
        print("[eQualle VIDEO][CLEAN] 🧹 Removed stray LLM 'Video' section.")
    return new_md
...
def main() -> None:
    ...
    if find_video_for_article:
        ...
        try:
            ...
            if v and isinstance(v, dict) and v.get("id"):
                ...
                video_payload = v
                print(f"[eQualle VIDEO][OK] ✅ Selected {v.get('id')}")
            else:
    ...
    print(f"[eQualle PROMPT][OK] ✅ {len(prompt)} chars.")
    ...
    if video_payload:
        ...
        print(f"[eQualle VIDEO][EMBED] ✅ Strategy: {strategy}")
        try:
