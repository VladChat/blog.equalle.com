...
def main() -> None:
    ...
    # === Новый блок: извлечение первой брендовой картинки ===
    image_url = None
    m = re.search(r"https://blog\.equalle\.com/images/brand/[^\s)\"']+\.(?:webp|jpg|jpeg|png)", article_md)
    if m:
        image_url = m.group(0)
        print(f"[eQualle IMAGE][OK] 🖼️ Found brand image → {image_url}")
    else:
        image_url = "https://blog.equalle.com/images/brand/01.webp"
        print("[eQualle IMAGE][MISS] ⚠️ No brand image found — fallback applied.")
